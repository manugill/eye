...
class App {
    ...
    io.on('connection', socket => {
      ...
      socket.on('load-url', url_string => {
        if (typeof url_string === 'string') {
          if (
            url_string.substr(0, 7) !== 'http://' &&
            url_string.substr(0, 8) !== 'https://'
          ) {
  ...
  setupWindow() {
    ...
    this.win.webContents.on('did-fail-load', () =>
      this.sendToClient('did-fail-load'),
    );
    this.win.webContents.on('did-start-loading', () =>
      this.sendToClient('did-start-loading'),
    );
    this.win.webContents.on(
      'did-stop-loading',
      () => this.sendToClient('did-stop-loading') && this.resetScrollBar(),
    );
    this.win.webContents.on(
      'did-navigate',
      (event, url) =>
        this.sendToClient('did-navigate', url) && this.resetScrollBar(),
    );
