...
const _getControllerIndex = inputSource =>
...
function init() {
  ...
  controllerMeshes = [_makeControllerMesh(), _makeControllerMesh()];
  ...
  // i have no idea what this does, iframes don't have a d attribute
  // but it has to be specified and has to be exactly 2
  iframe.d = 2;

  iframe.width = planeWidth;
  iframe.height = planeHeight;
  ...
  planeMesh = (() => {
    ...
    mesh.projectMouse = (x, y) => {
      ...
      plane.setFromCoplanarPoints(leftLine.start, leftLine.end, topLine.end);
      ...
      const intersectionPoint = raycaster.ray.intersectPlane(
        plane,
        localVector,
      );

      if (intersectionPoint) {
        const leftIntersectionPoint = leftLine.closestPointToPoint(
          intersectionPoint,
          true,
          localVector2,
        );

        const topIntersectionPoint = topLine.closestPointToPoint(
          intersectionPoint,
          true,
          localVector3,
        );
        ...
        const xFactor =
          topIntersectionPoint.distanceTo(topLine.start) / planeWorldWidth;
        const yFactor =
          leftIntersectionPoint.distanceTo(leftLine.start) / planeWorldHeight;
  ...
  boxMeshes = [_makeBoxMesh(), _makeBoxMesh()];
...
function animate(timestamp, frame) {
  try {
    ...
    for (let i = 0; i < inputSources.length; i++) {
      ...
      controllerMesh.matrix.decompose(
        controllerMesh.position,
        controllerMesh.quaternion,
        controllerMesh.scale,
      );
      ...
      raycaster.set(
        controllerMesh.position,
        localVector.set(0, 0, -1).applyQuaternion(controllerMesh.quaternion),
      );
      const intersectionPoint = raycaster.ray.intersectPlane(
        plane,
        localVector,
      ...
      for (let i = 0; i < gamepads.length; i++) {
        ...
        if (gamepad) {
          ...
          if (padPressed) {
            ...
            planeMesh.boxMesh.position.y =
              (-y / planeHeight) * planeWorldHeight;
...
if (navigator.xr) {
  (async () => {
    ...
    session.onselect = e => {
      // const coord = planeMesh.projectMouse(e.clientX, e.clientY);
      // console.log('mousemove', coord);
      // if (coord) {
      //   iframe.sendMouseMove(coord.x, coord.y);
      // }

      console.log('select', e); // XXX
    };
